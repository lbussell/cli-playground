...
public sealed class CommandLineGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        foreach (var optTypeName in cmd.OptionsTypeNames)
        {
            if (optionsTypes.TryGetValue(optTypeName, out var optType))
            {
                ...
                builder.AppendLine($"foreach (var arg in {mapperName}.Arguments) command.Add(arg);");
                builder.AppendLine($"foreach (var opt in {mapperName}.Options) command.Add(opt);");
            }
        }
    ...
    private static void GenerateOptionsMapper(SourceProductionContext ctx, OptionsTypeInfo info)
    {
        ...
        // Generate Arguments and Options properties
        if (arguments.Count > 0)
        {
            var argList = string.Join(", ", arguments.Select(a => $"{a.PropertyName}Argument"));
            builder.AppendLine($"public static IEnumerable<Argument> Arguments {{ get; }} = [{argList}];");
        }
        else
        {
            builder.AppendLine("public static IEnumerable<Argument> Arguments { get; } = [];");
        }

        if (options.Count > 0)
        {
            var optList = string.Join(", ", options.Select(o => $"{o.PropertyName}Option"));
            builder.AppendLine($"public static IEnumerable<Option> Options {{ get; }} = [{optList}];");
        }
        else
        {
            builder.AppendLine("public static IEnumerable<Option> Options { get; } = [];");
        }
